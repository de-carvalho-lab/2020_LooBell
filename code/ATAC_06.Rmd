...
```{bash eval=FALSE}
findMotifsGenome.pl data/03_augmented/04_atac_homer/stim_unique.bed hg19 data/03_augmented/04_atac_homer/01_stimVsAll -size given -p 10 -bg data/03_augmented/04_atac_homer/atac_library.bed
findMotifsGenome.pl data/03_augmented/04_atac_homer/dac_unique.bed hg19 data/03_augmented/04_atac_homer/01_dacVsAll -size given -p 10 -bg data/03_augmented/04_atac_homer/atac_library.bed
```

## 3. Visualising the results of the HOMER motif enrichment analysis

```{r}
homer <- read.table(file = file.path(r.config$dirs$augmented.data.dir,"04_atac_homer","01_dacVsAll","knownResults.txt"), header = FALSE, sep = "\t", skip = 1, col.names = c("Motif","Consensus","P-value","Log P-value","q-value","N FG","PC FG","N BG","PC BG"), stringsAsFactors = FALSE)
```

```{r}
index <- which(x = homer$q.value <= 0.05)[1:6]
name <- homer$Motif[index]
name <- substr(x =name, start = 1, stop = 4)
ylab <- expression(paste("-Log"[10],"(P-value)"))
log.p <- homer$Log.P.value
log.p <- log.p * -1
point.col <- rep("black",nrow(homer))
point.col[which(homer$q.value < 0.05)] <- "grey"
plot(x = 1:nrow(homer), y = log.p, pch = 20, col = point.col, xlab = "Rank", ylab = ylab, las = 1)
